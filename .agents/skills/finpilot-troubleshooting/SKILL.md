...
| Ghostty renders a serif font with odd letter spacing; `fc-list` shows the configured family installed | The from-source ghostty build (`build/30-cosmic-desktop.sh`) only discovers font files at most one directory below a font root — deeper nesting (e.g. `fonts/nerd-fonts/<family>/*.ttf`) is visible to fontconfig but not to ghostty, which then falls back to Noto Serif | Keep image fonts ≤1 level under `/usr/share/fonts` (`build/20-dotfiles.sh` extracts to `nerd-fonts-<family>/`). Diagnose with `ghostty +list-fonts` (ghostty's real discovery) vs `fc-list`, and `ghostty +show-face --cp=0x41` for the resolved face |
| Omarchy browser launch (Super+Shift+Enter) silently fails although `xdg-settings get default-web-browser` is correct | Upstream `omarchy-launch-browser` searches only `{~/.local,~/.nix-profile,/usr}/share/applications` for the browser's desktop entry and execs its first `Exec=` token — Flatpak browsers live in the flatpak export dirs and their Exec wraps `flatpak run`, so resolution fails on a flatpak-first image | Lateralus override `omarchy-overrides/bin/omarchy-launch-browser` resolves entries across flatpak export dirs and launches via `flatpak run <X-Flatpak app id>` |
